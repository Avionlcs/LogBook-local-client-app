...
const fileMap = {
'src/api/sales/complete/completeSale.controller.js': // Controller: complete sale const service = require("./completeSale.service"); module.exports = async function completeSaleController(req, res) { try { const result = await service.completeSale(req.body || {}); res.status(200).json({ success: true, message: "Sale completed successfully", sale: result }); } catch (e) { res.status(e.http || 500).json({ success: false, error: e.public || "Server error" }); } };.trim() + "\n",

'src/api/sales/complete/completeSale.service.js': // Service: orchestrates validation→idempotency→tx→finalize const db=require("../../../config/dbConfig"),{withRepeatableRead}=require("./db/withRepeatableRead"),{validateRequest}=require("./validators/request.validator"),lock=require("./repos/lockSale.repo"),items=require("./repos/getItems.repo"),totals=require("./repos/computeTotals.repo"),sumReq=require("./repos/sumRequestPayments.repo"),apply=require("./repos/applyPayments.repo"),finalize=require("./repos/finalizeSale.repo"),listPays=require("./repos/listPayments.repo"),idem=require("./repos/idempotency.repo"),{toNum,round2}=require("./utils/money"); async function w(c,i){ const s=await lock(c,i.sale_public_id); if(!s)throw h(404,"Sale not found"); if(s.status==="sold")throw h(409,"Sale already completed"); const t=await totals(c,i.sale_public_id); const it=await items(c,i.sale_public_id); const r=await sumReq(c,i.payments); const np=toNum(s.total_paid_amount||0)+toNum(r); if(np<toNum(t.total_amount))throw h(400,"Insufficient payment"); await apply(c,i.sale_public_id,i.payments); const row=await finalize(c,i.sale_public_id,t.total_amount,np,i.payments); const pays=await listPays(c,i.sale_public_id); const change_due=round2(np-toNum(t.total_amount)); return {...row,items:it,payments:pays,change_due}; } module.exports.completeSale=async function(i){ const ok=validateRequest(i); if(!ok.valid)throw h(400,ok.error); if(i.idempotency_key){ const c=await idem.find(i.sale_public_id,i.idempotency_key); if(c)return c; } const pool=db.getPool(); const r=await withRepeatableRead(pool,cl=>w(cl,i)); if(i.idempotency_key)await idem.save(i.sale_public_id,i.idempotency_key,r); return r; }; function h(code,msg){ const e=new Error(msg); e.http=code; e.public=msg; return e; }.trim() + "\n",

'src/api/sales/complete/validators/request.validator.js': // Validate request payload const { isValidMoney } = require("../utils/money"); const { ensure } = require("./payment.validator"); const ALLOWED = new Set(["cash","card","qr"]); module.exports.validateRequest = (b) => { if (!b || !b.sale_public_id) return { valid:false, error:"sale_public_id required" }; const a=b.payments; if (!Array.isArray(a)||a.length===0) return { valid:false, error:"payments required" }; if (a.length>10) return { valid:false, error:"too many payments" }; for(const p of a){ if(!p||!ALLOWED.has(p.method)) return { valid:false, error:"invalid payment method" }; if(p.currency&&p.currency!=="LKR") return { valid:false, error:"unsupported currency" }; if(!isValidMoney(p.amount)) return { valid:false, error:"invalid amount" }; if(!ensure(p)) return { valid:false, error:"invalid payment data" }; } if(b.idempotency_key&&typeof b.idempotency_key!=="string") return { valid:false, error:"invalid idempotency_key" }; return { valid:true }; };.trim() + "\n",

'src/api/sales/complete/validators/payment.validator.js': // (Reserved) Extra per-payment validation hook module.exports.ensure = (p) => { // Add business rules (lengths, meta limits) if needed return true; };.trim() + "\n",

'src/api/sales/complete/utils/money.js': // Money utils: strict 2dp, positive function toNum(x){ return typeof x==="number"?x:Number(x); } function isValidMoney(x){ if (x===null||x===undefined||x==="") return false; const s=String(x); if (!/^\\d+(?:\\.\\d{1,2})?$/.test(s)) return false; return Number(s)>0; } function round2(n){ return Math.round((+n+Number.EPSILON)*100)/100; } module.exports={ isValidMoney, toNum, round2 };.trim() + "\n",

'src/api/sales/complete/db/withRepeatableRead.js': // Transaction helper: REPEATABLE READ async function withRepeatableRead(pool, fn){ const client = await pool.connect(); try { await client.query("BEGIN"); await client.query("SET TRANSACTION ISOLATION LEVEL REPEATABLE READ"); const r = await fn(client); await client.query("COMMIT"); return r; } catch(e){ try{ await client.query("ROLLBACK"); }catch(_){} throw e; } finally { client.release(); } } module.exports = { withRepeatableRead };.trim() + "\n",

'src/api/sales/complete/repos/lockSale.repo.js': // Lock sale row FOR UPDATE module.exports = (c, id) => c .query("SELECT * FROM sales WHERE public_id=$1 FOR UPDATE", [id]) .then(r => r.rows[0]);.trim() + "\n",

'src/api/sales/complete/repos/getItems.repo.js': // Fetch sale items module.exports = (c, id) => c .query("SELECT * FROM sale_items WHERE sale_public_id=$1 ORDER BY id", [id]) .then(r => r.rows);.trim() + "\n",

'src/api/sales/complete/repos/computeTotals.repo.js': // Compute sale total = items - offers module.exports = (c, id) => { const q = \
...
'src/api/sales/complete/repos/sumRequestPayments.repo.js': // Sum request payments via VALUES cast to NUMERIC(12,2) module.exports = (c, pays) => { const vals = pays.map((_,i)=>\($${i+1}::NUMERIC(12,2))`).join(",");
...
'src/api/sales/complete/repos/applyPayments.repo.js': // Apply payments to sales totals module.exports = (c, id, pays) => { let cash=0,card=0,qr=0,cardRef=null,qrRef=null; for(const p of pays){ if(p.method==="cash") cash+=+p.amount; else if(p.method==="card"){ card+=+p.amount; if(p.reference) cardRef=p.reference; } else if(p.method==="qr"){ qr+=+p.amount; if(p.reference) qrRef=p.reference; } } const sql=\
UPDATE sales SET
...
'src/api/sales/complete/repos/finalizeSale.repo.js': // Finalize sale row module.exports = (c, id, total, paid, pays) => { const set=new Set(pays.map(p=>p.method)); const method=set.size===1?[...set][0]:"mixed"; const sql=\
UPDATE sales SET
...
'src/api/sales/complete/repos/listPayments.repo.js': // List payments reconstructed from sales row module.exports = async (c, id) => { const r=await c.query( "SELECT cash_payment_amount,card_payment_amount,qr_payment_amount,card_payment_reference,qr_payment_reference FROM sales WHERE public_id=$1",[id] ); const row=r.rows[0]||{}; const out=[]; if(+row.cash_payment_amount>0) out.push({method:"cash",amount:Number(row.cash_payment_amount)}); if(+row.card_payment_amount>0) out.push({method:"card",amount:Number(row.card_payment_amount),reference:row.card_payment_reference||null}); if(+row.qr_payment_amount>0) out.push({method:"qr",amount:Number(row.qr_payment_amount),reference:row.qr_payment_reference||null}); return out; };.trim() + "\n",

'src/api/sales/complete/repos/idempotency.repo.js': // Idempotency cache const db = require("../../../../config/dbConfig"); async function ensure(pool){ await pool.query(\CREATE TABLE IF NOT EXISTS idempotency_cache(
...
'migrations/create_sales_schema.js': module.exports = { async create(pool) { // 0) Extensions (safe if rerun) await pool.query(\CREATE EXTENSION IF NOT EXISTS "pgcrypto";`);
...
'migrations/create_inventory_schema.js': module.exports = { async create(pool) { // Enable pgcrypto extension for gen_random_uuid() await pool.query(\CREATE EXTENSION IF NOT EXISTS "pgcrypto";`);
