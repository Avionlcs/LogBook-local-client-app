...
export class UpdateItemQuantityService {
  ...
  private adjust(body: any): Observable<any> {
    console.log('🔼 Sending update-item request:', body); // debug log
    return this.http
  ...
  setQty(
    ...
    if (Number(nextQty) <= 0) {
      return this.dec(saleid, itemid, Math.max(Number(currentQty) || 0, 1), true);
    }
    if (delta > 0) {
      return this.inc(saleid, itemid, delta, mode);
    }
    if (delta < 0) {
      return this.dec(saleid, itemid, Math.abs(delta), false);
    }
